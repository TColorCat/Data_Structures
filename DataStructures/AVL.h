...
namespace Data_structures
{
	/*
	* 获取树的高度
	关于高度，有的地方将"空二叉树的高度是-1"，
	而本文采用维基百科上的定义：树的高度为最大层次。即空的二叉树的高度是0，
	...
	*/
	...
	AVLTreeNode<T>* AVLTree<T>::rightRightRotation(AVLTreeNode<T>*k1)
	{
		...
		k2->height = max(height(k2->left), k1->height) + 1;//这个k1.height是个省事的活
	...
	*/
	...
	/*
	* 将结点插入到AVL树中，并返回根节点
	* 把点插入的过程与二叉树一样，然后按照原路返回，遇到不平衡的位置就调整
	* 参数说明：
	*     tree AVL树的根结点
	*     key 插入的结点的键值
	* 返回值：
	...
	*/
	...
	AVLTreeNode<T>* AVLTree<T>::insert(AVLTreeNode<T>* &tree, T key)
	{
		...
		else if (key < tree->key) // 应该将key插入到"tree的左子树"的情况
		{
			...
			// 插入节点后，若AVL树失去平衡，则进行相应的调节。运行到这里就已经是插入后了。
